...
%>
<h1>Novo Participante</h1><br/>
<form action="participanteServlet" method="POST">
    <input type="hidden" name="acao" value="salva"/>
    <input type="hidden" name="idParticipante" value='<%=request.getParameter("idParticipante")!=null?request.getParameter("idParticipante"):"" %>'/> <br/>
    Nome: <input type="text" name="nome" value="<%=participante!=null?participante.getNome():"" %>"/> <br/>
    Cargo: <input type="text" name="cargo" size="8" value="<%=participante!=null?participante.getCargo():"" %>"/> <br/>
    ...
    <option value="<%= projeto.getIdProjeto() %>"
            <%
                if (participante.getProjetos().contains(projeto)) out.print("selected");
            %>
    >
...
<br><a href="index.jsp"><b>Menu</b></a>
</body>
</html>
